...
def data_loader_worker(tasks_queue, output_queue, quit_workers_event):
    ...
    while True:
        ...
        try:
            ...
            st_json_data = [{} for _ in range(len(env_json_data))]  # FIXME
...
def process_frame(
    ...
    if attack_is_stuck:
        step_env_data["mouse"]["buttons"] = [
            button for button in step_env_data["mouse"]["buttons"] if button != 0
        ]
    action, is_null_action = json_action_to_env_action(step_env_data)
    # Update hotbar selection
    current_hotbar = step_env_data["hotbar"]
...
class DataLoader:
    ...
    def __len__(self):
        ...
        for unique_id in self.unique_ids:
            ...
            total += len(json_data)
        return int(total / self.batch_size) * self.n_epochs
